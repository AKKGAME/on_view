<div class="min-h-screen bg-[#05050A] text-white pb-20 relative overflow-x-hidden font-sans selection:bg-purple-500 selection:text-white">
    ...
    <div class="relative z-20 px-4 py-3 md:py-4 border-b border-white/5 bg-[#05050A]/80 backdrop-blur-md sticky top-0">
        <div class="max-w-[1800px] mx-auto flex items-center justify-between">
            ...
            <div class="md:hidden">
                <span class="px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-bold">
        ...
        <div class="flex flex-col lg:flex-row">
            ...
            <div class="flex-1 w-full min-w-0 lg:py-8 lg:pl-8 lg:pr-6">
                ...
                <div class="relative group w-full">
                    ...
                    <div class="relative w-full aspect-video bg-black lg:rounded-2xl overflow-hidden shadow-2xl lg:ring-1 ring-white/10 z-10">
                        @if($isUnlocked)
                            <iframe src="{{ route('stream.play', $episode->id) }}" 
                                    class="w-full h-full" 
                                    frameborder="0" 
                                    allowfullscreen>
                            </iframe>
                        @else
                            <div class="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0a0a] text-center p-4" 
                                ...
                                {{-- STATE 1: Default Lock --}}
                                <div x-show="!showConfirm" class="relative z-10 flex flex-col items-center max-w-sm">
                                    <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-800 to-black border border-yellow-500/30 flex items-center justify-center shadow-2xl mb-4 animate-pulse">
                                        <svg class="w-8 h-8 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                    </div>
                                    <h2 class="text-2xl font-black text-white mb-1">PREMIUM</h2>
                                    <p class="text-slate-400 text-sm mb-6">Unlock to watch this episode.</p>

                                    @auth
                                        <button @click="showConfirm = true" class="w-full py-3 px-6 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-xl transition transform active:scale-95 shadow-lg shadow-yellow-500/20">
                                            UNLOCK ({{ $episode->coin_price }} G)
                                        </button>
                                    @else
                                        <a href="{{ route('login') }}" class="w-full py-3 px-6 bg-white/10 text-white font-bold rounded-xl border border-white/10 block">
                                            Login Required
                                        </a>
                                    @endauth
                                </div>
                ...
                <div class="mt-4 px-4 lg:px-0 flex flex-col md:flex-row gap-4 justify-between items-start">
                    <div class="w-full md:w-auto">
                        <h1 class="text-xl md:text-2xl font-bold text-white leading-snug line-clamp-2">{{ $episode->title }}</h1>
                        <div class="flex items-center gap-2 mt-2 text-xs md:text-sm text-slate-400 font-medium">
                    ...
                    <div class="w-full md:w-auto flex items-center gap-2 bg-[#1a1a1a] p-1 rounded-xl border border-white/5">
                        ...
                        <button @if($prev) wire:click="$dispatch('link-navigate', {href: '{{ route('anime.watch', $prev->id) }}'})" @else disabled @endif
                           onclick="@if($prev) window.location.href='{{ route('anime.watch', $prev->id) }}' @endif"
                        ...
                        <button @if($next) wire:click="$dispatch('link-navigate', {href: '{{ route('anime.watch', $next->id) }}'})" @else disabled @endif
                           onclick="@if($next) window.location.href='{{ route('anime.watch', $next->id) }}' @endif"
                ...
                {{-- COMMENTS SECTION (Visible on Desktop, Pushed down on Mobile) --}}
            ...
            {{-- On Mobile: Appears AFTER video info, BEFORE comments --}}
            <div class="w-full lg:w-[380px] shrink-0 lg:py-8 lg:pr-8 flex flex-col">
                
                {{-- Playlist Container --}}
                <div class="mt-6 lg:mt-0 px-4 lg:px-0">
                    <div class="bg-[#0f0f13] border border-white/5 lg:rounded-2xl rounded-xl overflow-hidden flex flex-col shadow-xl lg:sticky lg:top-24 h-auto max-h-[400px] lg:max-h-[calc(100vh-120px)]">
                        ...
                        <div class="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                            @foreach($playlist as $ep)
                                ...
                                <a href="{{ route('anime.watch', $ep->id) }}" wire:navigate
                                   ...
                                   {{ $isActive ? 'bg-purple-600/10 border-purple-500/50' : 'hover:bg-white/5 hover:border-white/5' }}">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <h4 class="text-xs lg:text-sm font-bold truncate transition {{ $isActive ? 'text-purple-300' : 'text-slate-300 group-hover:text-white' }}">
                                            {{ $ep->title }}
                                        </h4>
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="text-[9px] font-bold px-1.5 py-0.5 rounded 
                                                {{ $ep->is_premium ? 'bg-yellow-500/10 text-yellow-500' : 'bg-green-500/10 text-green-500' }}">
                                                {{ $ep->is_premium ? 'PREMIUM' : 'FREE' }}
                                            </span>
                                            @if($isActive)
                                                <span class="text-[9px] font-bold text-purple-400 uppercase tracking-wider">Playing</span>
                                            @endif
                                        </div>
                                    </div>
                                </a>
                ...
                {{-- MOBILE COMMENTS (Visible only on mobile, after playlist) --}}
                <div class="lg:hidden px-4 mt-8 border-t border-white/5 pt-6">
                    <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                        <span class="text-purple-500">#</span> Comments
                    </h3>
                    @livewire('episode-comments', ['episodeId' => $episode->id])
                </div>
